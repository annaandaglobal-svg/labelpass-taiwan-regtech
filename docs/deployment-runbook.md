...
This check compares table counts and probes high-value Taiwan aliases such as `MSG`, `味精`, `카제인나트륨`, `魷魚`, `奇異果`, `화장품 효능표현`, `화장품 금지성분`, `限用成分`, `防腐劑`, `防曬成分`, `식품 효능표현`, `food claim substantiation`, `slightly sweet`, `甜味宣稱`, `醫療效能`, `輸入食品查驗`, `HS 0307 health certificate`, and `食品業者登錄`.
