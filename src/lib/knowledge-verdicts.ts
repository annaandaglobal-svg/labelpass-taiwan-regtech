...
function baseVerdictForKnowledgeTerm(term: KnowledgeTermForVerdict): KnowledgeVerdict | null {
  ...
  if (category === "hair_dye_ingredient") {
    return {
      ...
      detail:
        "PPD(p-페닐렌디아민)·톨루엔-2,5-디아민·레조르시놀 등 산화 염모 성분은 대만 特定用途(染髮) 화장품 성분으로, 인정 목록·혼합 후 최대 함량 이내에서만 쓸 수 있습니다(예: PPD 2% — 혼합 후 free base 기준). 라벨에 알레르기·패치테스트, 만 16세 미만 금지, 속눈썹·눈썹 염색 금지 경고가 의무입니다(정확 문구는 衛福部 공고 기준). 제품 등록·PIF도 필요합니다.",
      tone: "gold",
      state: "needs_check",
  ...
  if (category === "cosmetic_ingredient_restriction") {
    return {
      ...
      detail:
        "대만 化粧品成分使用限制表에 함량·조건·경고문이 정해진 제한 성분입니다. 확정: 살리실산 2%(일반)·3%(린스오프)·방부용 0.5% + '3세 미만 사용 금지'(샴푸 제외) 경고; AHA는 %상한 없이 pH≥3.5 + 자외선 민감·'사용 후 자외선 차단제' 경고; 붕산 0.1%(3세 미만 금지·삼킴 금지). 2027-10-01 시행: 레티놀 0.05%(바디 리브온)·0.3%(기타) RE + '비타민A 함유' 문구, 알파-아르부틴 2%(얼굴)·0.5%(바디) + 하이드로퀴논 불순물 ≤20ppm.",
      tone: "gold",
      state: "needs_check",
  ...
  if (category === "food_safety_contaminant") {
    return {
      ...
      detail:
        "곰팡이독소·벤조피렌·중금속·농약·동물용의약품·히스타민·방사능은 대만 위생표준(食品中污染物質及毒素衛生標準 L0040138 등)에 한도가 있고, 수입 국경검사에서 초과하면 반려·회수됩니다. 확정 한도(예): 총 아플라톡신 10 µg/kg(일반)·곡류가공 4·땅콩원료 15, 아플라톡신 M1 0.025 µg/kg(영유아 조제식); 히스타민 200 mg/kg(히스티딘 높은 어류)·400 mg/kg(염장·발효 어류/魚醬). 견과·곡류·향신료(아플라톡신), 훈제·구이(벤조피렌), 수산(히스타민·중금속), 축산(잔류항생제)이 고위험입니다. 시험성적서로 적합성을 확보하세요.",
      tone: "gold",
      state: "needs_check",
