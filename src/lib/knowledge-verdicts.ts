...
function baseVerdictForKnowledgeTerm(term: KnowledgeTermForVerdict): KnowledgeVerdict | null {
  ...
  if (category === "trans_fat_pho_ban") {
    return {
      label: "부분경화유(PHO) 금지·트랜스지방 표시",
      detail:
        "대만은 2018-07-01부터 식품에 不完全氫化油(부분경화유, PHO) 사용을 금지합니다(食用氫化油之使用限制, 食安法 §15). 마가린·쇼트닝·식물성 크림/奶精·베이커리 등 PHO로 배합된 제품은 수입·판매가 불가하며 위반 시 NT$3만~300만입니다. 또 포장식품 영양표시에 트랜스지방 함량 표기가 의무이고, '트랜스지방 0'은 100g당 0.3g 이하 등 조건을 충족할 때만 표기할 수 있습니다. (정제·유제품 유래 미량 트랜스지방은 허용 — 인공 PHO만 금지)",
      tone: "red",
      state: "restricted_risk",
      uncertainty: "제품에 부분경화유(PHO)가 들어있는지 배합을 확인하세요. 있으면 대만 수입·판매가 금지됩니다.",
      chips: ["不完全氫化油 금지", "마가린·쇼트닝·奶精", "2018 시행", "트랜스지방 0 조건"],
      actions: [
        "부분경화유(PHO)를 완전경화유·비경화 유지로 대체하세요(대만 식품 사용 금지).",
