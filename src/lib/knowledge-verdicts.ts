...
function baseVerdictForKnowledgeTerm(term: KnowledgeTermForVerdict): KnowledgeVerdict | null {
  ...
  if (category === "hair_dye_ingredient") {
    return {
      ...
      chips: ["染髮 특정용도", "눈썹·속눈썹 금지", "피부시험 권장", "혼합 후 한도"],
  ...
  if (category === "cosmetic_gmp") {
    return {
      ...
      chips: ["化粧品GMP(ISO22716)", "일반 화장품 2026-07-01", "OEM 적합성 확인", "PIF와 별개"],
  ...
  if (category === "pesticide_border_risk") {
    return {
      ...
      chips: ["잔류농약 집중검사", "克美素·陶斯松·2,4-D", "업체 수입중단 사례", "시험성적 필수"],
  ...
  if (category === "infant_food_tariff") {
    return {
      ...
      chips: ["영유아용 1901.10 = 5%", "일반 조제분유 = 12%", "소매·조성 조건", "配方은 査驗登記"],
  ...
  if (category === "inorganic_arsenic") {
    return {
      label: "무기비소(無機砷) 한도·검사 — 한국↔대만 수치 불일치 주의",
      detail:
  ...
  if (category === "food_enzyme") {
    return {
      label: "식품 효소 — 허용 급원(source)만 인정",
      detail:
        "대만에서 식품 효소는 첨가물·가공보조제로 '허용된 급원(미생물·동식물)'에서 제조된 것만 쓸 수 있고, 생산 균주 자체가 식품 허용이어야 합니다. 이 효소들(트랜스글루타미나아제·브로멜라인·파파인·락타아제·펙티나아제·글루코아밀라아제)은 널리 쓰이나 대만 공식 목록 등재·허용 급원을 食品添加物 酵素 목록/食品原料 플랫폼에서 확인해야 합니다. 트랜스글루타미나아제는 대만 등록(Streptomyces mobaraensis 급원)이 확인됩니다. 가다랑어(鰹魚) 등은 식품이나 가쓰오부시 '추출물' 형태의 원료 적격성은 별도 확인이 필요합니다.",
      tone: "gold",
      state: "needs_check",
      uncertainty: "효소는 대만 허용 급원(생산 균주)에서 제조됐는지, 食品添加物 酵素 목록·원료 플랫폼 등재를 확인하세요 — 급원이 비허용이면 불가합니다.",
      chips: ["허용 급원만 인정", "생산 균주 식품허용", "酵素 목록 확인", "추출물은 별도 확인"],
      actions: ["효소의 생산 급원(균주·동식물)이 대만 식품허용인지, 食品添加物 酵素 목록/食品原料 플랫폼 등재를 확인하고 급원·규격 서류를 확보하세요."]
    };
  }
  ...
  if (category === "exfoliating_acid_aha") {
    return {
      ...
      detail:
        "만델릭애씨드(苦杏仁酸, AHA)·락토바이오닉애씨드(乳糖酸, PHA)는 대만 화장품 금지·제한 목록에 없어 허용으로 보입니다(정확한 INCI로 최신 목록 확인 권장). 대만은 각질제거 AHA 화장품에 총 농도 상한(최대 18%, 2024-07에 10%→18% 상향)·pH 조건·자외선 민감 주의 프레임워크를 두므로(pH·경고 세부는 확인 필요), AHA(만델릭 등) 제품은 농도·pH·자외선 경고문을 확인하세요. PHA(락토바이오닉)는 분자가 커 침투가 낮아 자외선 경고 요건이 완화됩니다.",
      tone: "gold",
      state: "needs_check",
      uncertainty: "만델릭·락토바이오닉은 허용으로 보이나, AHA(만델릭) 제품은 대만 총 농도 상한(≤18%)·pH·자외선 경고 요건을 확인하세요.",
      chips: ["AHA 최대 18%(2024↑)", "pH·자외선 경고", "PHA는 완화", "최신 목록 확인"],
      actions: ["만델릭 등 AHA는 총 농도(≤18%)·pH·자외선 경고문을 확인하고, 정확한 INCI로 최신 化粧品成分使用限制表를 대조하세요."]
    };
  }
  ...
  if (category === "cosmetic_general_active") {
    return {
      ...
      detail:
        "대만 화장품은 네거티브 리스트라 금지 성분표(禁止使用成分表)·제한 성분표(成分使用限制表)에 없으면 허용(법정 함량 한도 없음)입니다. 이 성분들(소듐PCA·페룰산·이데베논·아르지렐린(아세틸헥사펩타이드)·에칠아스코빌에터 등)은 검색상 두 목록에서 확인되지 않아 일반 허용으로 추정되나, 최신 化粧品成分使用限制表(2027-10 개정판 포함)에서 정확한 INCI로 재확인하고 제품 통보 등록을 하세요. ⚠️ 예외: '미백'을 표방하는 성분(예: 에칠아스코빌에터)은 대만 허용 미백성분(약 13종) 목록에 있어야 하며 없이 미백을 표방하면 약품 클레임이 됩니다. 이데베논 등은 고용량·의료 효능 표방 시 약품 경계라 분류 확인이 필요합니다.",
      tone: "gold",
      state: "needs_check",
  ...
  if (category === "functional_fiber") {
    return {
      ...
      chips: ["차전자피 승인·10.2g/일", "車前子 경고문 필수", "곤약젤리 질식경고", "분리·가교는 非傳統성"],
      actions: [
  ...
  if (category === "polyglutamic_acid") {
    return {
      ...
      chips: ["화장품 허용(보습제)", "식품첨가물 용도별 한도", "분리원료는 非傳統성 확인", "효능표방=健康食品"],
      actions: [
  ...
  if (category === "alcohol_import_regime") {
    return {
      ...
      chips: ["재정부 관할·수입허가", "매건 酒類查驗", "메탄올/SO₂ 한도", "주세(도수당)"],
  ...
  if (category === "commodity_tax_beverage") {
    return {
      label: "음료 화물세(貨物稅) 15% — 관세·부가세와 별개",
      detail:
  ...
  if (category === "food_raw_material_eligibility") {
    return {
      ...
      chips: ["목록 없으면 불가", "한약재 식약경계", "非傳統성 사전승인", "균주·곤충 제한"],
  ...
  if (category === "irradiated_food") {
    return {
      ...
      chips: ["허용품목만 조사", "라두라 마크", "輻射照射處理 표시", "향신료·건조식품"],
  ...
  if (category === "food_contact_migration") {
    return {
      ...
      chips: ["대만 溶出 재시험", "KFDA 성적 불가", "멜라민 고온시험", "도자기 Pb/Cd"],
  ...
  if (category === "food_contact_labeling") {
    return {
      ...
      chips: ["용기 材質·耐熱溫度 표시", "재활용코드(한국마크 불가)", "국경검사 대상", "F코드 2026-04 개정"],
  ...
  if (category === "industrial_dye_adulterant") {
    return {
      ...
      chips: ["蘇丹紅 즉시 수입중단", "구리엽록소(오일 금지)", "디메틸옐로(두부)", "검출=반려"],
  ...
  if (category === "plasticizer_food") {
    return {
      label: "가소제(塑化劑 DEHP) — 음료·젤리·보충제 집중검사",
      detail:
      ...
      chips: ["DEHP 塑化劑", "음료·젤리·보충제 표적", "포장 이행 점검", "起雲劑 사태"],
  ...
  if (category === "illegal_processing_agent") {
    return {
      ...
      chips: ["마레인산 전분(毒澱粉)", "붕사(떡·어묵)", "과산화수소 표백", "吊白塊 포름알데히드"],
  ...
  if (category === "net_content_metrology") {
    return {
      ...
      chips: ["淨含量·允差(BSMI)", "고형량 별도표시", "℮ 마크 무효", "음의 허용오차"],
  ...
  if (category === "nutrient_content_claim") {
    return {
      ...
      chips: ["강조표시 수치기준", "無糖 0.5g·低鈉 120mg", "아스파탐 PKU 경고", "폴리올 완사 주의"],
  ...
  if (category === "cosmetic_drug_boundary") {
    return {
      ...
      detail:
        "일부 K-뷰티 기능성 성분은 대만에서 화장품이 아니라 藥品(약품, 藥事法)으로 규제돼 화장품 등록이 무효이고 통관이 막힙니다: 하이드로퀴논(미백약), 트레티노인/레티노산, 미녹시딜(생발), 케토코나졸(비듬약), 벤조일퍼옥사이드·아다팔렌(여드름약), 화장품 상한을 넘는 고농도 살리실산. 대만의 허용 미백성분은 알부틴·코직산·나이아신아마이드·아스코르빌글루코사이드 등 포지티브 리스트이며, 목록 밖 성분으로 미백을 표방하면 약품 클레임으로 간주됩니다. 등록 전 성분을 화장품 성분제한표와 대조하고, 약품 성분은 제거·재처방하거나 애매하면 TFDA 分類分級 질의를 하세요.",
      tone: "red",
      state: "restricted_risk",
  ...
  if (category === "inspection_escalation") {
    return {
      ...
      chips: ["1회 실패→100%", "원산지+CCC 단위", "해제=5배치·3배량", "複驗 1회뿐"],
  ...
  if (category === "origin_marking_fraud") {
    return {
      ...
      chips: ["원산지 세탁 처벌", "3배 벌금·몰수", "실질적 변형 요건", "貿易法 §28"],
  ...
  if (category === "tariff_classification_duty") {
    return {
      ...
      chips: ["한국=1란(MFN)", "세번이 관세 좌우", "무역진흥비·부가세", "稅則預先審核"],
  ...
  if (category === "dairy_fat_naming") {
    return {
      label: "버터·크림·마가린 품명·유지방 기준 (乳品 개편 예정)",
      detail:
