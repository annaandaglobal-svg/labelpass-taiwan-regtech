...
const sourceOpsMetadataPath = path.join(root, "data", "knowledge", "source-ops-metadata.json");
...
const persistedSourceOpsMetadata = JSON.parse(await readFile(sourceOpsMetadataPath, "utf8"));
...
for (const result of index.results) {
  ...
  const sourceOps = sourceOpsById.get(result.id) ?? null;
    ...
    `insert into public.knowledge_snapshots (source_key, content_hash, fetched_at, from_cache, bytes, text_chars, document_path, extract, metadata) values (` +
      [
        ...
        jsonSql({
          ...
          source_ops_metadata_version: sourceOpsMetadata.generated_from?.source_registry_version ?? null,
          authority: result.authority,
          domain: result.domain,
          tags: result.tags,
          source_ops: sourceOps,
          languages: sourceOps?.languages ?? [],
          review_owner: sourceOps?.review_owner ?? null,
          selector_strategy: sourceOps?.selector_strategy ?? null,
          date_strategy: sourceOps?.date_strategy ?? null,
          refresh_strategy: sourceOps?.refresh_strategy ?? null,
          evidence_policy: sourceOps?.evidence_policy ?? null,
